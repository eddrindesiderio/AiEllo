#!/usr/bin/env python3
"""
Streamlit Web App for Your Personal AI (OpenAI GPT)
A web interface for your personal AI assistant powered by OpenAI GPT models.

Run with: streamlit run deployment/streamlit_app_openai.py
"""
...
# Page configuration
st.set_page_config(
    page_title="Personal AI Assistant - OpenAI GPT",
    page_icon="🤖",
    layout="wide",
...
def get_fallback_response(message):
    """Get fallback response when OpenAI API is not available"""
...
What specific math topic would you like help with?"""
            ...
            return "I can help with simple math problems. For complex calculations, I need my OpenAI GPT API for full functionality."
...
def main():
    ...
    st.subheader("Powered by OpenAI GPT-3.5-turbo")
    ...
    with st.sidebar:
        ...
        if openai_available:
            ...
            st.info("Using GPT-3.5-turbo for intelligent responses and advanced code generation")
        else:
            st.error("❌ OpenAI GPT Not Connected")
        ...
        - **General Questions**: Wide range of topics with GPT intelligence
        ...
        st.markdown("---")
        st.markdown("### 🔥 OpenAI GPT Features:")
        st.markdown("""
        - **Advanced Reasoning**: GPT-3.5-turbo intelligence
        - **Code Generation**: Complete, working code examples
    ...
    if prompt := st.chat_input("Ask me anything..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking with OpenAI GPT..."):
                if openai_available:
                    try:
                        # Get system prompt
                        ...
                        ai_response = call_openai_api(messages, system_prompt)
                        
                        # Add debug info if needed
                        if st.session_state.get("debug_mode", False):
                            ai_response = f"🔍 DEBUG: OpenAI GPT Success\n\n{ai_response}"
                            
                    except Exception as e:
                        st.error(f"OpenAI GPT Error: {e}")
                        ai_response = f"🔧 API Error - using fallback response:\n\n{get_fallback_response(prompt)}"
                else:
                    ai_response = f"🔧 No OpenAI GPT - using fallback response:\n\n{get_fallback_response(prompt)}"
    ...
    # Additional info
    st.markdown("---")
    st.markdown("### 🚀 OpenAI GPT Advantages:")
